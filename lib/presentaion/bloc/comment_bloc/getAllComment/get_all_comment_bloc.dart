...
class GetCommentsBloc extends Bloc<GetCommentsEvent, GetCommentsState> {
  ...
  FutureOr<void> commentsFetchEvent(
    CommentsFetchEvent event, Emitter<GetCommentsState> emit) async {
  emit(GetCommentsLoadingState());
  final Response? result = await PostRepo.getAllCommentPost(postId: event.postId);

  if (result == null) {
    emit(GetCommentsServerErrorState(error: 'Failed to fetch comments'));
    return; // Exit the method if the result is null
  }
