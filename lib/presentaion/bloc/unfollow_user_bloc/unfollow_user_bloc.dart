...
class UnfollowUserBloc extends Bloc<UnfollowUserEvent, UnfollowUserState> {
  ...
  Future<void> _followUser(
    ...
    try {
      final Response? response = await PostRepo.followUser(followeesId: event.followersId);
  ...
  Future<void> _unfollowUser(
    ...
    emit(UnfollowUserLoadingState());
    try {
      final Response? response = await PostRepo.unFollowUser(followeesId: event.followersId);
