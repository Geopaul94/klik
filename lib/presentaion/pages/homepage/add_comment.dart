        ...
        body: MultiBlocListener(
          listeners: [
              ...
              listener: (context, state) async {
                if (state is DeleteCommentLoadingState) {
                  await showDialog(
                    context: context,
                    builder: (context) {
                      return const Center(child: SpinningLinesExample());
                    },
                    barrierDismissible: false,
                  );
                } else if (state is DeleteCommentSuccesfulState) {
                  Navigator.of(context).pop();
                  FocusScope.of(context).unfocus();
               //   widget.onCommentDeleted(); // Update count in parent widget
                    context.read<GetCommentsBloc>().add(CommentsFetchEvent(postId: widget.id)); customSnackbar(context, "Comment Deleted Successfully", green);
                } else if (state is DeleteCommentServerErrorState) {
                  Navigator.of(context).pop();
                  customSnackbar(context, "Server error occurred!", red);
                }
            ...
            BlocListener<CommentPostBloc, CommentPostState>(
              listener: (context, state) {
                if (state is CommentPostSuccesfulState) {
                  widget.onCommentAdded(); // Update count in parent widget
                ...
                child: BlocConsumer<GetCommentsBloc, GetCommentsState>(
                  ...
                  builder: (context, state) {
                    if (state is GetCommentsLoadingState) {
                      ...
                      return state.comments.isEmpty
                          ? const Center(child: Text("No Comments", style: TextStyle(color: Colors.white)))
                          : ListView.builder(
                              itemCount: state.comments.length,
                              itemBuilder: (context, index) {
                                  ...
                                  child: ListTile(
                                    leading: CircleAvatar(
                                      backgroundImage: comment.user.profilePic.isNotEmpty
                                          ...
                                          : const AssetImage('assets/g_logo.png') as ImageProvider,
                                    ),
                                    title: Text(
                                      comment.user.userName,
                                        ...
                                        ? IconButton(
                                            ...
                                            onPressed: () async {
                                              ...
                                              if (currentUser == comment.user.id) {
                                                ...
                                                if (confirmDelete == true) {
                                                  context.read<DeleteCommentBloc>().add(DeleteCommentButtonClickEvent(commentId: comment.id));
                                                }
                                              } else {
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        if (_formKey.currentState!.validate()) {
                          context.read<CommentPostBloc>().add(CommentPostButtonClickEvent(
                            userName: widget.userName,
                            postId: widget.id,
                            content: _commentController.text,
                          ));
